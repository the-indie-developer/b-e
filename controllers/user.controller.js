...
const verifyNewUserOtpAndSignIn = async (req,res)=>{
  try {
    ...
    const newUser = new User({email,fullname,role,password:hashedPassword})
...
The Meri Pehchaan Team`
    ...
    return res.status(200).send({
      success:true,
      message:'User verification and creation successfull !'
    })
...
const loginUser = async (req,res)=>{    
  try {
    ...
    if (!email || !password) {                  // BASIC CHECKING OF CREDENTIALS 
      return res.status(400).send({
        success:false,
...
const checkOtp = async (req, res) => {
  try {
    ...
    if (!user || !user.resetPasswordOTP || user.resetPasswordExpire < Date.now()) {
      return res.status(400).send({
        success: false,
...
export { loginUser, forgotPassword, checkOtp, resetPassword, getProfilePic, genNewUserOtp, verifyNewUserOtpAndSignIn,handleProfileUpdate}
